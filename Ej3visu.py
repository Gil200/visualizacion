...
with tab1:
    ...
    with col1:
        fig1 = px.box(filtered_df, x="listing_type", y="minimum_nights", title="Minimum nights by listing type", color_discrete_sequence=["#FF5733"])
    ...
    fig4 = px.box(filtered_df, x="listing_type", y="reviews_per_month", title="Reviews per Month by Listing Type", color_discrete_sequence=["#2ECC71"])
    st.plotly_chart(fig4)
    fig5 = px.scatter(filtered_df, x="availability_365", y="price", color="listing_type", title="Availability vs Price")
    st.plotly_chart(fig5)
    fig6 = px.box(filtered_df, x="listing_type", y="minimum_nights", title="Estimated Number of People by Listing Type (Proxy: Minimum Nights)", color_discrete_sequence=["#F39C12"])
